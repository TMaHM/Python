...
from test_config import Phone


class PhoneFunction(Phone):
    """
    进行实例化时，需要一个参数：cur_exec_file,即前执行文件的文件名，由os.path.basename(__file__)提供
    Usage::
    ...
    def __init__(self, ip, extension, cur_exec_file, log_level='info', **kw):
        Phone.__init__(ip, extension, **kw)
    ...
    def idle_state_set(self):
        ...
        url_f4 = self.url_keyboard + 'F4'
        url_x = self.url_keyboard + 'X'

        log.info(self.ip + ' try to return to [Idle] status...')
        try:
            ...
            check_status = self.check_status('idle')
            if check_status:
                return True
            else:
                return False
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if r.status_code == 200:
                    ...
                    log.info('Try capture screen 2nd times failed, please check the pwd.')
            else:
                log.info(
